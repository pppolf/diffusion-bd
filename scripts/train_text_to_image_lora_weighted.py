...
class TrainPreprocessor:
    def __init__(self, tokenizer, image_column, caption_column, image_transforms):
...
def collate_train_examples(examples):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " or to a folder containing files that Hugging Face Datasets can understand."
        ),
    )
    parser.add_argument(
        "--dataset_config_name",
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of batches prefetched by each worker when workers are enabled.",
    )
    parser.add_argument(
        "--sampling_flag_column",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Sampling weight for rows where sampling_flag_column is true.",
    )
    ...
    parser.add_argument(
        "--save_lora_each_epoch",
        action="store_true",
        help="Save inference-ready LoRA weights under epoch-N after every epoch.",
    )
    parser.add_argument(
...
def main():
    ...
    train_transforms = transforms.Compose(transform_steps)
    ...
    preprocess_train = TrainPreprocessor(
        tokenizer,
        image_column,
        caption_column,
        train_transforms,
    ...
    with accelerator.main_process_first():
        ...
        if args.sampling_flag_column is not None:
            ...
            flags = dataset["train"][args.sampling_flag_column]
            sample_weights = torch.tensor(
                [args.sampling_positive_weight if bool(flag) else 1.0 for flag in flags],
                dtype=torch.double,
            )
            positive_count = sum(bool(flag) for flag in flags)
            if positive_count == 0:
                raise ValueError(
                    f"Sampling column `{args.sampling_flag_column}` contains no positive rows."
                )
            ...
            effective_rate = (
                positive_count * args.sampling_positive_weight / sample_weights.sum().item()
            )
            logger.info(
                "Weighted sampling enabled: %s positives, weight %.4f, effective rate %.4f",
                positive_count,
                args.sampling_positive_weight,
                effective_rate,
            )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if args.save_lora_each_epoch:
            accelerator.wait_for_everyone()
            if accelerator.is_main_process:
                epoch_dir = os.path.join(args.output_dir, f"epoch-{epoch + 1}")
                epoch_unet = unwrap_model(unet)
                epoch_lora_state = convert_state_dict_to_diffusers(
                    get_peft_model_state_dict(epoch_unet)
                )
                StableDiffusionPipeline.save_lora_weights(
                    save_directory=epoch_dir,
                    unet_lora_layers=epoch_lora_state,
                    safe_serialization=True,
                )
                logger.info("Saved epoch LoRA weights to %s", epoch_dir)
